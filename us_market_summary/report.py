...
- 하락/상승 종목 목록에는 미국 상장 종목만 포함 (한국 종목은 코멘트로만 언급, 절대
  단독 항목으로 넣지 말 것)

## 출력 형식 — 텔레그램 HTML(parse_mode=HTML)로 그대로 전송 가능한 형태여야 함
- 마크다운 표(|---|) 절대 금지. 대신 "• 종목명: 등락률 — 사유" 같은 글머리 기호 줄로 작성
- 굵게는 <b>...</b>, 기울임은 <i>...</i> 만 사용. 그 외 HTML 태그(표, 링크 등) 금지
- 이모지 제목/구분은 그대로 사용 (📊 🔴 🟢 🔥 📅 💡)
- 아래 섹션 순서를 지킬 것:
  1. 📊 [날짜] 미국 증시 마감 요약 (제목)
  2. 지수 등락 (S&P500/Nasdaq/Dow/Russell2000/SOX — 종가, 등락률)
  3. 섹터 등락 상위/하위
  4. 상승/하락 배경 2~3줄
